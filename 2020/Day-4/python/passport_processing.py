...
def readFile() -> list:
    result = list()
    with open(f"{__file__.rstrip('passport_processing.py')}input.txt", "r") as f:
        for data in f.read()[:-1].split("\n\n"):
            result.append(dict(d.split(":") for d in data.replace("\n", " ").split()))
    return result


def cheeck_validity(infos: list) -> bool:
    return all((info in infos for info in ("byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid")))

def part1(data: list) -> int:
    for i in data:
        if cheeck_validity(i):
...
if __name__ == "__main__":
    ...
    print(f"Part 2: {part2(valid_passport)}")
